...
def apply_qwen35_q4_lm_prefill_linear_patch() -> bool:
    ...
    if attn_cls is not None and attn_cls.__call__ is not attn_wrapper:
        ...
        def patched_attention(self, x, mask=None, cache=None):
            ...
            # Resolve SDPA per call, never at patch time: a snapshot taken
            # here would keep routing quantized caches into a stale SDPA that
            # reads the group_size only quantized caches carry (issue #2372).
