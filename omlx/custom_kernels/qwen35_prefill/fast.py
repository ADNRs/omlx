...
_nax_attn_cache: bool | None = None


def nax_attn256_available() -> bool:
    """True when the fused NAX head-dim-256 attention kernel is usable.

    Requires the rebuilt binding, NAX hardware, and a NAX metallib that
    actually ships the dsplit attention pipelines (the C++ probe creates
    the pipeline once). OMLX_FA256_NAX=0 disables the route (testing).
    """
    global _nax_attn_cache
    if _nax_attn_cache is None:
        env = os.environ.get("OMLX_FA256_NAX", "").strip().lower()
        if env in ("0", "false", "off"):
            _nax_attn_cache = False
        else:
            _nax_attn_cache = (
                _ext is not None
                and hasattr(_ext, "qwen35_attn256_nax")
                and hasattr(_ext, "nax_attn_kernels_built")
                and bool(_ext.is_nax_available())
                and bool(_ext.nax_attn_kernels_built())
            )
        if _nax_attn_cache:
            logger.info("Qwen FA-256 NAX dsplit attention kernel available")
    return _nax_attn_cache


def nax_attn256_demote() -> None:
    """Disable the NAX attention route after a failed launch."""
    global _nax_attn_cache
    _nax_attn_cache = False


def nax_attn256_runtime_active() -> bool:
    return bool(
        _ext is not None
        and hasattr(_ext, "nax_attn_runtime_active")
        and _ext.nax_attn_runtime_active()
    )


def qwen35_attn256_nax(
    q: mx.array,
    k: mx.array,
    v: mx.array,
    scale: float,
    ...
    *,
    ...
    if _ext is None or not hasattr(_ext, "qwen35_attn256_nax"):
        raise RuntimeError("qwen35_attn256_nax native kernel is unavailable")
    return _ext.qwen35_attn256_nax(
        q,
        k,
        v,
        scale,
        causal=causal,
        dispatch_budget=int(dispatch_budget),
