...
def test_reconstruct_preserves_variable_length_arrays_state():
...
class TestCanonicalLayerCacheTypes:
    ...
    collapse types that change tensor representation (a packed 4-bit KV
    cache and KVCache store different tensor formats) — collapsing those
    ...
    def test_unknown_type_not_collapsed(self):
        result = BlockAwarePrefixCache._canonical_layer_cache_types(
            ["ArraysCache", "LegacyQuantKVCache", "KVCache"]
        )
        # Unrecognized types must remain distinct from plain KVCache.
        assert "LegacyQuantKVCache" in result
